...
| FEAT-026 | 检索过滤 | SearchPage.vue（标签范围） | /search、/chat 请求体加性 document_ids/tag | （无新表，复用 document_tags） | ⚠️ 存量修改点：retriever/chroma/bm25（全通道过滤）——见 FEAT-026 文档 |
| FEAT-027 | 证据兜底 | ChatPage.vue（证据不足徽标） | （无新端点；chat 响应/done 帧加性 evidence_level） | （无新表） | ⚠️ 拒答类文案豁免数据表；config 开关 ENABLE_EVIDENCE_GUARD |
| FEAT-028 | 全局问答 | GraphGlobalPage.vue + ChatPage.vue（预填） | `/api/graph` 前缀 communities、communities/rebuild | graph_communities | ⚠️ 另用 Chroma 第二集合 community_summaries；rebuild 不占查询闸（ADR-010） |
| FEAT-029 | 评测 judge 模板修缮 | （无页面，EvalRunsPage 早已自动展示 judge 指标） | （无新 API） | （无新表） | ⚠️ 提示词来源重构豁免三件套（见 FEAT-029 文档） |

> 数据表完整清单（15 张）：`users`、`documents`、`chunks`、`document_tags`、`conversations`、`messages`、`message_feedback`、`message_sources`、`progress_history`、`embedding_cache`、`eval_cases`、`eval_runs`、`entity_aliases`、`graph_communities`、`schema_version`（`sqlite_sequence` 为 SQLite 内部表）。
