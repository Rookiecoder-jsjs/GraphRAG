...
class LLMService:
    ...
    async def extract_entities_and_relations_batch(
        ...
        # Text lives in templates/entity_extraction.md (GUIDE-003 T2-1) so
        # prompt iterations flow through the eval gate (军规②), not code.
        system_prompt = load_prompt(
            "entity_extraction",
            entity_types=", ".join(entity_types),
            relation_types=", ".join(_RELATION_TYPES),
        )
