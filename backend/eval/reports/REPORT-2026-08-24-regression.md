...
1. ~~**RRF 融合后 chunk 去重缺失**~~ **【2026-08-25 勘误：本条为误报，已销】**
   当时的诊断是"ch8-memory 的 retrieved 前 5 名里同一 chunk_id（`34396fb2…`）
   出现 3 次，融合层未按 chunk_id 归并"。逐层复核（fusion.py 本就按 doc_id
   键累加分数天然归并；扩展层另有 seen_ids 二次去重）并真跑 ch8-memory 原
   查询验证：返回 8 个 chunk 零重复。所谓"重复"实为**同文档兄弟块的 id 公共
   前缀被截断显示造成的误读**——真实 id 是 `34396fb2…_1`、`_2`、`_3` 和裸
   `34396fb2…`，四个不同的子块。无需修复。
   （教训：报告里的 chunk_id 应写全量或至少含 `_N` 后缀，截断显示会误导
   事后诊断。）
2. **runner 不保存 per-case JSON 明细**：--markdown 模式下 retrieved 列表
   不落盘，事后诊断需要重跑。建议 runner 增加 `--out <file>` 双格式落盘。
   （上条误报之所以难排查，正是缺 per-case 明细——本项优先级随之上升。）
